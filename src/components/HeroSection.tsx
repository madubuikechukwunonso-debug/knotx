...
export default function AdminOverviewSection() {
  ...
  // Save Videos (with safe overwrite)
  const handleSaveVideos = async () => {
    ...
    try {
      ...
      alert("All 4 Hero Videos saved successfully! (Previous ones replaced)");
  ...
  // Save Images (with safe overwrite)
  const handleSaveImages = async () => {
    ...
    setUploading(true);
    try {
      // Delete only old Gallery Images
      ...
      alert("All 3 Gallery Images saved successfully! (Previous ones replaced)");
    ...
    <div className="space-y-8 bg-[#0f172a] min-h-screen p-6 text-white">
      ...
      <div className="bg-[#1e2937] border border-slate-700 rounded-3xl p-8">
        ...
        <p className="text-xs text-slate-500 mt-6 text-center">
          Uploading a new set will safely replace the previous one.
        </p>
      </div>
    </div>
  );
