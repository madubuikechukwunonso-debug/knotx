// src/sections/admin/MessageConversationTabs.tsx
'use client';
  ...
  if (conversationList.length === 0) {
    return (
      <div className="bg-card border border-border rounded-3xl p-12 text-center">
        ...
        <p className="text-foreground text-lg">No messages yet</p>
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <div className="lg:w-80 flex-shrink-0">
        <div className="bg-card border border-border rounded-3xl shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-border bg-muted">
            <p className="font-medium text-foreground text-sm">Conversations ({conversationList.length})</p>
          </div>
          <div className="divide-y divide-border max-h-[600px] overflow-y-auto">
            {conversationList.map((conv) => (
              ...
              >
                <div className="h-10 w-10 rounded-2xl bg-muted flex items-center justify-center text-foreground flex-shrink-0">
      ...
      <div className="flex-1 min-w-0">
        {activeConversation ? (
          <div className="bg-card border border-border rounded-3xl shadow-sm flex flex-col h-[700px]">
            {/* Conversation Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-muted rounded-t-3xl">
              <div className="flex items-center gap-4">
                <div className="h-12 w-12 rounded-2xl bg-primary flex items-center justify-center text-primary-foreground text-xl font-medium">
                  {activeConversation.name.charAt(0).toUpperCase()}
                </div>
                <div>
                  <p className="font-semibold text-foreground text-lg">{activeConversation.name}</p>
                  <p className="text-muted-foreground text-sm flex items-center gap-1">
                    <span className="font-mono text-xs bg-muted px-1.5 py-0.5 rounded">ID:</span> {activeConversation.email}
                  </p>
                </div>
              </div>
              ...
              {[...activeConversation.allMessages].reverse().flatMap((msg) => {
                ...
                thread.push(
                  <div key={`msg-${msg.id}`} className="flex justify-start">
                    <div className="max-w-[75%] bg-card border border-border rounded-3xl rounded-tl-none p-4 shadow-sm">
                      <div className="flex items-center gap-2 text-xs text-muted-foreground mb-1">
                        <span className="font-semibold text-foreground">{msg.name}</span>
                        <span className="text-muted-foreground">•</span>
                        <span>{new Date(msg.createdAt).toLocaleString('en-CA')}</span>
                      </div>
                      {msg.subject && <p className="font-medium text-foreground mb-1">{msg.subject}</p>}
                      <p className="text-foreground whitespace-pre-wrap text-sm">{msg.message}</p>
                    </div>
                  </div>
                );
                // Replies
            ...
            <div className="p-4 border-t border-border bg-card rounded-b-3xl">
              <form action={sendReply} className="flex gap-3">
                ...
                <textarea
                  ...
                  className="flex-1 rounded-3xl border border-border bg-background px-5 py-3 text-sm focus:outline-none focus:border-primary resize-y min-h-[52px] text-foreground"
                  required
                />
                <button
                  type="submit"
                  className="self-end bg-primary hover:bg-primary/90 text-primary-foreground px-8 rounded-3xl flex items-center justify-center gap-2 text-sm font-medium h-[52px]"
                >
                  <Reply size={18} />
                  Send
                </button>
              </form>
              <p className="text-[10px] text-muted-foreground mt-2 text-center">Press Send to reply • End Chat clears this conversation permanently</p>
            </div>
          </div>
        ) : (
          <div className="bg-card border border-border rounded-3xl p-12 text-center h-[700px] flex items-center justify-center">
            <div>
              <MessageCircle className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
              <p className="text-foreground">Select a conversation from the left</p>
            </div>
          </div>
        )}
      </div>
