    ...
    <div className="space-y-8 max-w-7xl mx-auto px-4 bg-background text-foreground">
      ...
      <div className="bg-card rounded-3xl border border-border p-6 sm:p-8">
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h2 className="text-2xl sm:text-3xl font-serif">Shop Categories</h2>
              <p className="text-sm text-muted-foreground mt-1">Organize your shop products beautifully</p>
            </div>
            <div className="text-xs px-4 py-2 bg-muted text-foreground rounded-full font-medium self-start sm:self-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              <input
                ...
                className="w-full border border-border bg-background rounded-2xl px-5 py-4 text-base focus:outline-none focus:border-primary placeholder:text-muted-foreground"
                required
              />
            </div>
            <button
              type="submit"
              className="w-full sm:w-auto bg-primary hover:bg-primary/90 text-primary-foreground px-8 py-4 rounded-2xl font-medium text-base transition-all active:scale-[0.985]"
            >
              Create Category
            </button>
          </div>
        ...
        <div>
          <h3 className="font-semibold text-lg mb-4 text-foreground">Your Categories</h3>
          ...
          {productCategories.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {productCategories.map((cat) => (
                <div 
                  key={cat.id} 
                  className="group bg-card border border-border rounded-3xl p-6 hover:shadow-xl hover:border-primary/30 transition-all duration-300 active:scale-[0.985]"
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="w-14 h-14 bg-primary rounded-2xl flex items-center justify-center text-primary-foreground text-2xl flex-shrink-0">
                      📁
                    </div>
                    <form action={deleteProductCategory}>
                      ...
                      <button
                        type="submit"
                        className="text-red-400 hover:text-red-600 p-2 -mr-2 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                      >
                  ...
                  <h4 className="font-semibold text-xl text-foreground mb-2 pr-8">{cat.name}</h4>
                  
                  <div className="flex items-center gap-2">
                    <span className="inline-block px-3 py-1 bg-muted text-foreground rounded-full text-xs font-medium">
      ...
      <div className="bg-card rounded-3xl border border-border p-6 sm:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            <h2 className="text-2xl sm:text-3xl font-serif">Shop Preview</h2>
            <p className="text-sm text-muted-foreground mt-1">How customers see your categories</p>
          </div>
          <div className="text-xs px-4 py-2 bg-muted text-foreground rounded-full font-medium self-start sm:self-auto">
            Live Preview
          </div>
        ...
        {productCategories.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {productsByCategory.map((cat) => {
              ...
              return (
                <div 
                  key={cat.id} 
                  className="group relative bg-card border border-border rounded-3xl overflow-hidden hover:shadow-2xl transition-all duration-500 active:scale-[0.985]"
                >
                    ...
                    {firstProductImage ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center">
                          ...
                          <p className="text-muted-foreground text-sm font-medium">No products yet</p>
                        </div>
                      </div>
                    )}
                    
                    <div className="absolute top-4 right-4 bg-card/95 backdrop-blur-sm px-4 py-1.5 rounded-full shadow-lg border border-border">
                  ...
                  <div className="p-6">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <h3 className="font-semibold text-xl text-foreground leading-tight pr-2">
                        {cat.name}
                      </h3>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-primary via-primary/80 to-primary transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500" />
                </div>
              );
            })}
            
            {uncategorizedProducts.length > 0 && (
              <div className="group relative bg-card border border-border rounded-3xl overflow-hidden hover:shadow-2xl transition-all duration-500">
                <div className="relative h-56 sm:h-64 bg-muted flex items-center justify-center">
                  <div className="text-center">
                    <div className="text-6xl mb-3 opacity-40">📦</div>
                    <p className="text-muted-foreground text-sm font-medium">Uncategorized</p>
                  </div>
                </div>
                <div className="p-6">
                  <h3 className="font-semibold text-xl text-foreground mb-2">Other Products</h3>
          ...
          <div className="text-center py-16 bg-muted rounded-3xl">
            ...
            <h3 className="text-xl font-semibold text-foreground mb-2">Ready to organize your shop?</h3>
            <p className="text-muted-foreground max-w-md mx-auto">Create categories above and watch your products come to life in beautiful collections</p>
          </div>
        )}
      </div>
    </div>
